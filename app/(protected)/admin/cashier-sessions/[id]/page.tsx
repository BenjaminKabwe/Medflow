  ...
  return (
    <div className="p-6 flex flex-col gap-6 min-h-screen max-w-5xl mx-auto">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 dark:text-slate-500">
              ...
              <span className="flex items-center gap-1">
                ...
                Ouverte le {format(new Date(session.opened_at), "d MMM yyyy · HH:mm", { locale: fr })}
              </span>
              {session.closed_at && (
                <span className="flex items-center gap-1">
                  <Calendar className="w-3 h-3" />
                  Clôturée le {format(new Date(session.closed_at), "d MMM yyyy · HH:mm", { locale: fr })}
                </span>
              )}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 px-5 py-4 flex items-center justify-between">
        <div>
          <p className="text-xs text-slate-400 uppercase tracking-widest mb-0.5">Total encaissé</p>
          <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">
            {grandTotal.toLocaleString("fr-CD")} USD
          </p>
        </div>
        <div className="text-right">
          <p className="text-xs text-slate-400 mb-0.5">Paiements</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/50">
                  <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">N°</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Patient</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden md:table-cell">Date</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden sm:table-cell">Mode</th>
                  <th className="px-5 py-3 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Montant</th>
                </tr>
              </thead>
